...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnDept30EmpsOrderedBySalaryDesc()
    {
        ...
        List<Emp> result = emps.Where(emp => emp.DeptNo == 30).OrderBy(emps => emps.Sal).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(emp => grades.Where(s => emp.Sal >= s.Losal && emp.Sal <= s.Hisal)
            .Select(s => new {emp.EName, s.Grade})).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo)
            .Select(g => new {DeptNo = g.Key, AvgSal = g.Average(emp => emp.Sal)}).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal > emps.Where(d => d.DeptNo == emp.DeptNo)
                .Average(d => d.Sal))
            .Select(e => e.EName)
            .ToList();  
